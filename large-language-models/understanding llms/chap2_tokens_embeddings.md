...
We will then dive into the famous word-to-vec embedding method that preceded modern-day LLMs and see how it's extending the concept of token embeddings to build commercial recommendation systems that power lot of apps that we use.
...
but tokens aren't the only output of a model they are alos the way in which model sees its inputs.

The text prompt sent to a model is first broken down into tokens.

### How tokenizers prepare input for the language model

If we see from the outside, generative LLMs can take an input prompt and generate a response.
...
before the propmt is presented to the language model, however it first has to go though a tokjenizerthat breaks it into pieces.
...
lets look at the code example and interact with these tokens ourselves.
here we'll be downloading an LLm and intercating how to tokenize the input before generating text with the LLM.
